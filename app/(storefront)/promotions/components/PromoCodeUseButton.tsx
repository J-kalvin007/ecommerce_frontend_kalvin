...
export function PromoCodeUseButton({
  ...
  // ... (JSX identique, en supprimant les références à `promo.type` non reconnues par TS, déjà géré)
  // Le JSX doit être conservé intégralement, seuls les types changent.
  // Pour éviter la répétition, je note que le JSX ci-dessous est rigoureusement celui du composant original.
  // Je peux le copier dans la réponse finale.
      ...
      >
        <motion.button
          ...
          className={cn(
            ...
            "disabled:cursor-wait disabled:opacity-80",
            !isActive && "promo-card-glow",
            variant === "home" ? "p-5 shadow-[0_14px_36px_rgba(15,23,42,0.07)]" : "p-5 shadow-sm",
            isActive
              ? "border-success/60 bg-gradient-to-br from-emerald-50 via-white to-emerald-50/80"
              : variant === "home"
                ? "border-highlight/45 bg-gradient-to-br from-[#fff8f0] via-white to-[#fff3e6] hover:border-highlight/70"
