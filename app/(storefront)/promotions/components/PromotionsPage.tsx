...
// Image placeholder si aucun produit en vedette
const FALLBACK_HERO = "/assets/images/LOGO.png"; // à adapter
...
function mapSaleToCard(sale: Soldes): PromoProductCard {
    return {
        id: sale.id,
        productId: sale.id, // ou l'ID du produit si disponible
        ...
        reviewCount: 0, // Soldes ne fournit pas ces infos, on peut les laisser à 0
        rating: 5.0,
    };
}
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                const sales: Soldes[] = salesRes.status === "fulfilled" && salesRes.value.ok ? salesRes.value.data : [];
                const codes: PromoCodeList[] = codesRes.status === "fulfilled" && codesRes.value.ok ? codesRes.value.data : [];
