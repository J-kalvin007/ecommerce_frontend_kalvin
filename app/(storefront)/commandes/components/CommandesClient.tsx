...
export default function CommandesClient() {
  ...
  // Chargement des données au montage de l'étape 3
  useEffect(() => {
    if (step === 3) {
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      if (res.ok) {
        ...
        alert("Erreur lors de la validation de l'adresse : " + (res.error?.message || ""));
      }
    } else {
      setStep(step + 1);
    }
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                >
                  <h2 className="mb-6 font-display text-xl font-bold">Informations de livraison</h2>
                  <div className="space-y-6 rounded-3xl p-6 shadow-sm" style={{ background: bgElevated, border: `1px solid ${border}` }}>
                    ...
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                      ...
                      <InputField label="Téléphone" type="tel" value={address.phone} onChange={(v) => updateAddress("phone", v)} required />
                    </div>

                    <div className="border-t pt-5" style={{ borderColor: border }}>
                      <label className="mb-3 block text-sm font-semibold">Localisation GPS & Adresse</label>
              ...
              <div className="mt-8 flex items-center justify-between border-t pt-8" style={{ borderColor: border }}>
                ...
                {step === 1 && (
                  <button
                    ...
                    className="flex items-center gap-2 rounded-xl bg-[#1f4d3f] px-8 py-3.5 font-bold text-white shadow-lg transition-all hover:bg-[#1f4d3f]/90 disabled:opacity-50"
                  >
                ...
                {step === 2 && (
                  <button
                    onClick={handleNext}
                    className="flex items-center gap-2 rounded-xl bg-[#1f4d3f] px-8 py-3.5 font-bold text-white shadow-lg transition-all hover:bg-[#1f4d3f]/90"
                  >
...
function InputField({ label, type = "text", value, onChange, placeholder, required }: any) {
  const { resolvedTheme } = useThemeStore();
  const isDark = resolvedTheme === "dark";
  ...
  return (
    <div>
      ...
      <input
        ...
        required={required}
        className="w-full rounded-xl py-3.5 px-4 text-sm outline-none transition-all focus:ring-2"
        style={{
          background: inputBg,
