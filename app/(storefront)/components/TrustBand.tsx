...
export default function TrustBand() {
  ...
  useEffect(() => {
    const fetchPromos = async () => {
      try {
        ...
        const displayItems = codes.map((c) =>
          c.description ? `${c.code} - ${c.description}` : c.code
        );
        setPromoItems(displayItems);
      } catch (error) {
  ...
  const loopingItems = [...promoItems, ...promoItems];
  ...
  return (
    <section className="border-y border-border bg-surface">
      ...
      {/* Bannière défilante des codes promo (affichée uniquement si des codes sont actifs) */}
      {promoItems.length > 0 && (
        <div className="overflow-hidden bg-primary text-white">
          <motion.div
            animate={{ x: ["0%", "-50%"] }}
