...
type PageState = 'idle' | 'submitting' | 'success' | 'error' | 'invalid_link';
...
export default function ResetPasswordConfirmPage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password1 !== password2) {
      setFieldErrors({ new_password2: "Les mots de passe ne correspondent pas." });
      return;
    }

    if (password1.length < 8) {
      setFieldErrors({ new_password1: "Minimum 8 caractères." });
      return;
    }
  ...
  const renderInvalidLink = () => (
    ...
    >
      <div className="relative mb-6 flex h-20 w-20 items-center justify-center">
        ...
        <ShieldOff className="relative z-10 h-9 w-9 text-red-500" />
      </div>
      <motion.h3 className="text-xl font-bold text-[#0F2D20]">
        Lien invalide ou expiré
      </motion.h3>
      <motion.p className="mt-2 max-w-sm text-sm leading-relaxed text-[#6b7a65]">
        {globalError || "Ce lien de réinitialisation n'est plus valide. Les liens expirent après 1 heure et ne peuvent être utilisés qu'une seule fois."}
      </motion.p>
      <motion.p className="mt-2 text-xs text-[#8a9685] italic">
        💡 Si vous avez reçu plusieurs emails, utilisez toujours le plus récent.
      </motion.p>
  ...
  const renderError = () => (
    ...
    >
      <div className="relative mb-6 flex h-20 w-20 items-center justify-center">
        <div className="absolute inset-0 rounded-full bg-red-50" />
        <AlertCircle className="relative z-10 h-9 w-9 text-red-500" />
      </div>
      <motion.h3 className="text-xl font-bold text-[#0F2D20]">
        Réinitialisation échouée
      </motion.h3>
      <motion.p className="mt-2 max-w-xs text-sm leading-relaxed text-[#6b7a65]">
        {globalError || "Une erreur est survenue lors de la réinitialisation."}
      </motion.p>
      <motion.div className="mt-7 flex gap-3">
        <button
          onClick={() => setPageState('idle')}
          className="rounded-xl border border-[#dde5d8] bg-[#f8faf6] px-6 py-3 text-sm font-semibold text-[#0F2D20] hover:bg-[#dde5d8]/50"
        >
          Réessayer
        </button>
        <Link
          href="/auth/forgot-password"
          className="group inline-flex items-center gap-2 rounded-xl bg-[#0F2D20] px-6 py-3 text-sm font-semibold text-white"
        >
          Nouveau lien
        </Link>
      </motion.div>
      ...
      <motion.form variants={stagger} initial="hidden" animate="visible" className="space-y-5" onSubmit={handleSubmit}>
        <motion.div variants={fadeUp}>
          ...
          <div className="relative">
            <Lock className={`absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 ${fieldErrors.new_password1 ? 'text-red-400' : 'text-[#9aab94]'}`} />
            <input
              ...
              className={`w-full rounded-xl border ${fieldErrors.new_password1 ? 'border-red-300 bg-red-50' : 'border-[#dde5d8] bg-[#f8faf6]'} py-3.5 pl-12 pr-12 text-sm text-[#0F2D20] outline-none transition-all placeholder:text-[#b0bfa9] focus:border-[#0F2D20] focus:shadow-[0_0_0_3px_rgba(15,45,32,0.08)] disabled:opacity-60`}
              required
              minLength={8}
            />
            <button
          ...
          {fieldErrors.new_password1 && (
            <p className="mt-1.5 text-xs text-red-500 font-medium flex items-center gap-1">
              <AlertCircle className="w-3 h-3" /> {fieldErrors.new_password1}
            </p>
          )}
        </motion.div>
        ...
        <motion.div variants={fadeUp}>
          ...
          <div className="relative">
            <Lock className={`absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 ${fieldErrors.new_password2 ? 'text-red-400' : 'text-[#9aab94]'}`} />
            <input
              ...
              className={`w-full rounded-xl border ${fieldErrors.new_password2 ? 'border-red-300 bg-red-50' : 'border-[#dde5d8] bg-[#f8faf6]'} py-3.5 pl-12 pr-12 text-sm text-[#0F2D20] outline-none transition-all placeholder:text-[#b0bfa9] focus:border-[#0F2D20] focus:shadow-[0_0_0_3px_rgba(15,45,32,0.08)] disabled:opacity-60`}
              required
              minLength={8}
            />
            <button
          ...
          {fieldErrors.new_password2 && (
            <p className="mt-1.5 text-xs text-red-500 font-medium flex items-center gap-1">
              <AlertCircle className="w-3 h-3" /> {fieldErrors.new_password2}
            </p>
          )}
        </motion.div>
          ...
          <div className="flex flex-col lg:flex-row h-full min-h-[540px]">
            ...
            >
              <AnimatePresence mode="wait">
                {pageState === 'invalid_link' && renderInvalidLink()}
                {pageState === 'error' && renderError()}
                {pageState === 'success' && renderSuccess()}
                {(pageState === 'idle' || pageState === 'submitting') && renderForm()}
              </AnimatePresence>
            </motion.div>
          </div>
        </motion.div>
