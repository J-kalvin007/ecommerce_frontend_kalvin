  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      <div className="w-full lg:w-5/12 space-y-4">
        ...
        <div className="rounded-2xl border border-border/60 bg-surface-alt/30 p-5 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input value={form.name} onChange={(e) => handleNameChange(e.target.value)} placeholder="Ex: Rouge, 500g..." className="h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" required />
            </div>
            
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Prix (FCFA) *</label>
                <input type="number" step="any" min="0" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" required />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Stock</label>
                <input type="number" min="0" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} className="h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} placeholder="Auto" className="h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Poids (g)</label>
                <input type="number" min="0" value={form.weight_grams} onChange={(e) => setForm({ ...form, weight_grams: e.target.value })} placeholder="0" className="h-10 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2 border-t border-border/50">
              {editingId && <button type="button" onClick={reset} className="rounded-xl border border-border bg-surface px-4 py-2 text-sm font-medium text-foreground hover:bg-surface-alt transition-colors">Annuler</button>}
      ...
      {/* Right Column: List */}
      <div className="w-full lg:w-7/12 rounded-2xl border border-border/60 bg-surface/30 overflow-hidden flex flex-col h-[500px]">
        <div className="bg-surface-alt/40 px-5 py-4 border-b border-border/50">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar">
          {variants.length > 0 ? (
            variants.map((v, idx) => (
              <div key={v.id || idx} className="group flex items-center justify-between rounded-xl border border-border bg-surface p-4 shadow-sm transition-all hover:border-primary/50 hover:shadow-md">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-sm text-foreground truncate">{v.name}</span>
                    <span className="rounded bg-surface-alt px-1.5 py-0.5 text-[10px] font-mono text-muted-foreground">{v.sku || "—"}</span>
                  </div>
                ...
                <div className="flex items-center gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity pl-4">
                  <button onClick={() => handleEdit(v)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-alt text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors">
                    <Edit3 className="h-4 w-4" />
                  </button>
                  <button onClick={() => onDeleteVariant(v.id)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-alt text-muted-foreground hover:bg-red-500/10 hover:text-red-500 transition-colors">
