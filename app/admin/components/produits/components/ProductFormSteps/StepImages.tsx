...
export function StepImages({ uploadedImages, existingImages, onAddImages, onRemoveUploaded, onSetPrimaryUploaded, altText, onAltTextChange }: StepImagesProps) {
  ...
  return (
    <div className="space-y-6">
      <div onClick={() => ref.current?.click()} className="flex h-40 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border bg-surface-alt transition-colors hover:border-primary hover:bg-primary/5">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4 sm:grid-cols-4 lg:grid-cols-6">
            {uploadedImages.map((img, idx) => (
              <div key={img.id} className="group relative aspect-square overflow-hidden rounded-2xl border border-border bg-surface">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4 sm:grid-cols-4 lg:grid-cols-6">
            {existingImages.map((img) => (
              <div key={img.id} className="relative aspect-square overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
                <Image 
      ...
      <div>
        ...
        <input value={altText} onChange={(e) => onAltTextChange(e.target.value)} className="h-11 w-full rounded-2xl border border-border bg-surface px-4 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20" placeholder="Description des images" />
      </div>
    </div>
  );
}
