...
export function LoyaltyTierProgressBar({
  ...
  const ptsForNext = nextTier ? nextTier.min_points : safePoints;
  const range = ptsForNext - ptsBase;
  const progress = range > 0 ? Math.min(((safePoints - ptsBase) / range) * 100, 100) : 100;
  const ptsNeeded = nextTier ? Math.max(nextTier.min_points - safePoints, 0) : 0;
  ...
  const maxPts = nextTier ? nextTier.min_points : safePoints;
    ...
    <div className="w-full space-y-3">
      ...
      <div className="flex items-center justify-between text-[11px] font-semibold">
        <span className={cn(cfg.textColor)}>
          {safePoints.toLocaleString("fr-FR")} pts
        </span>
        {nextTier ? (
      ...
      <div className="relative h-3 w-full">
        ...
        {sortedTiers
          ...
          .map((t) => {
            ...
            const reached = safePoints >= t.min_points;
            return (
              <div
                key={t.name}
